...
for (const count of [1, 4, 5, 9, 34, 100]) {
 ...
 try { const dry=parse(await core.card_bulk_create.execute({cards:Array.from({length:count},(_,i)=>({title:`Card ${i}`,correlationKey:`row-${i}`})),dryRun:true,format:'json'})); assert.equal(dry.data.count,count); assert.equal(dry.data.results.length,0); assert.equal(JSON.parse(readFileSync(dry.data.artifact.path,'utf8')).results.length,count); assert.equal(creates,0); assert.equal(queries,1); const apply=parse(await core.card_bulk_create.execute({cards:Array.from({length:count},(_,i)=>({title:`Card ${i}`})),dryRun:false,format:'json'})); assert.equal(apply.data.created,count); assert.equal(apply.data.results.length,0); assert.equal(JSON.parse(readFileSync(apply.data.artifact.path,'utf8')).results.length,count); assert.equal(creates,count); assert.equal(queries,2); } finally { globalThis.fetch=original; }
}
