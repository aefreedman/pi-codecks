...
The parent enables parsing only for its internal built-in provider invocation, requires exactly these three fields, and never accepts the envelope as credentials. User-configured external helpers retain v1 success-only semantics: their nonzero stdout is discarded and reported as a generic helper failure. The envelope contains no retry duration, secret reference, account, path, raw diagnostic, or manager metadata; it does not trigger an automatic retry. Recognition requires a nonzero `op` exit and the known bounded diagnostic phrase; a zero-exit warning, generic 429 text, or unknown failure is not positive rate-limit evidence.

The built-in provider also reports fixed, secret-safe configuration, timeout, cancellation, protocol, capacity, and unknown-helper failures. Authentication, permission, and missing-reference causes are not guessed from a generic exit code. Structured operation errors include provider/stage/category; `requestSent: false` is emitted only with operation accounting proving no Codecks dispatch. Earlier compound-operation dispatches are preserved, and unwrapped direct calls without accounting omit that claim. Third-party helpers retain their existing generic error contract.
