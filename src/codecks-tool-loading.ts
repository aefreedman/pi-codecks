...
export const CODECKS_SEARCH_CATALOG: readonly CodecksSearchCatalogEntry[] = [
  ...
  { name: "codecks_card_bulk_create", aliases: ["bulk create cards", "import cards", "csv card import"], tags: ["card", "bulk", "create", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first, review detailed normalized records and duplicate evidence, and apply only after explicit approval.", "Submit one approved bulk operation and let it pace or stop safely; do not manually chunk records or count requests to manage rate limits. Apply defaults to compact returned references; required blocks incomplete credential-visible evidence, while parent-scoped required dry-runs are no-create previews and required apply is blocked.", "Account fallback is only for semantic title-filter rejection, probe budget, or incomplete probes; transport/auth/rate/cancel/timeout/queue failures block. Records are strict: use assigneeId from codecks_user_lookup, never assignee."], activeSafety: `${MUTATION_OPERATION_SAFETY} Default to dryRun=true; review duplicate discovery completeness and policy before apply.` },
