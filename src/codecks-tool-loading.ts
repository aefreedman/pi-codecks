...
export const CODECKS_SEARCH_CATALOG: readonly CodecksSearchCatalogEntry[] = [
  ...
  { name: "codecks_card_bulk_create", aliases: ["bulk create cards", "import cards", "csv card import"], tags: ["card", "bulk", "create", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first, review detailed normalized records and duplicate evidence, and apply only after explicit approval.", "Apply defaults to compact returned references; choose duplicatePolicy=required for a blocking credential-visible duplicate guarantee or verification=identity only for meaningful identity read-back.", "Records are strict: use assigneeId from codecks_user_lookup, never assignee."], activeSafety: `${MUTATION_OPERATION_SAFETY} Default to dryRun=true; review duplicate discovery completeness and policy before apply.` },
