...
export const CODECKS_SEARCH_CATALOG: readonly CodecksSearchCatalogEntry[] = [
  ...
  { name: "codecks_card_update", aliases: ["update card", "edit card", "change card content", "assign milestone", "update card tags", "set card tags"], tags: ["card", "update", "content", "tags", "metadata", "mutation"], guidance: ["Update only explicitly requested card fields and inspect the target first when it is ambiguous.", CARD_REFERENCE_SAFETY], activeSafety: `${MUTATION_OPERATION_SAFETY} ${CARD_REFERENCE_SAFETY}` },
  ...
  { name: "codecks_card_bulk_create", aliases: ["bulk create cards", "import cards", "csv card import"], tags: ["card", "bulk", "create", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first and verify normalized write intent. Apply must pass that preview's previewFingerprint as expectedPreviewFingerprint; exact authorization can approve this matching apply without another interaction.", "Submit one approved operation; the package paces physical requests at 40 per five seconds. Do not manually chunk records or count requests.", "Use codecks_card_search for optional likely-match checks before approval. Bulk create does no duplicate discovery or post-create reads. An empty results array means there were no exceptional records; do not read the detail artifact after full success unless exact generated identities or diagnostics are needed."], activeSafety: `${MUTATION_OPERATION_SAFETY} Default to dryRun=true and review the requested writes before apply.` },
  { name: "codecks_card_bulk_update", aliases: ["bulk update cards", "import card updates", "csv card update", "batch run assignment", "batch parent assignment"], tags: ["card", "bulk", "update", "run", "parent", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first, then pass that previewFingerprint as expectedPreviewFingerprint for apply. Matching authorization covers that apply without another interaction.", "The package paces physical requests at 40 per five seconds and retries only explicit bounded HTTP 429 responses. continueOnError applies only to definitely rejected non-429 failures; ambiguous writes stop the batch."], activeSafety: `${MUTATION_OPERATION_SAFETY} Default to dryRun=true and apply only the matching fingerprinted preview after approval.` },
  ...
  { name: "codecks_velocity_observations_update", aliases: ["update velocity cache", "refresh velocity observations", "cache delivered cards", "velocity observation cache"], tags: ["run", "card", "velocity", "cache", "refresh", "observation"], guidance: ["Update the factual caller-owned cache once, then reuse it for reports. Incremental refresh defaults to a 10-day overlap."], activeSafety: "Writes one caller-owned workspace-contained JSON cache atomically; it does not apply analytical transformations." },
  { name: "codecks_velocity_report", aliases: ["velocity report", "statistical velocity report", "velocity percentiles", "mean p25 p50 p75", "capacity report", "team velocity"], tags: ["run", "velocity", "capacity", "report", "statistics", "percentile", "p25", "p50", "p75"], guidance: ["Consume an existing observation cache without querying Codecks, choose calendar_delivered or run_attributed explicitly, and inspect the expanded transformation manifest.", "CSV and Markdown outputs are independent workspace-contained artifacts."], activeSafety: "Reads a caller-owned observation cache and writes only explicitly requested workspace-contained report artifacts." },
...
export function searchCodecksTools(input: CodecksToolSearchInput, descriptions: ReadonlyMap<string, string> = new Map()): CodecksToolSearchMatch[] {
  ...
  return pair.map((name) => byName.get(name)).filter((entry) => Boolean(entry)).slice(0, limit) as CodecksToolSearchMatch[];
}
