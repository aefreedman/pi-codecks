...
export const CODECKS_SEARCH_CATALOG: readonly CodecksSearchCatalogEntry[] = [
  ...
  { name: "codecks_card_search", aliases: ["search cards", "find card", "disambiguate card", "list cards"], tags: ["card", "search", "discover", "inspect"], guidance: ["Use card search for disambiguation and scope discovery; deck or milestone filters infer their location.", "Prefer compact output, counts for aggregates, and detailed only when every row is required."], activeSafety: "Use for card disambiguation; compact output is the default and deck/milestone filters infer scope." },
  ...
  { name: "codecks_card_create", aliases: ["create card", "new card"], tags: ["card", "create", "mutation", "content"], guidance: ["Create a card only with explicit target and content intent.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} ${CARD_REFERENCE_SAFETY}` },
  { name: "codecks_card_set_parent", aliases: ["set parent", "hero parent", "make subcard", "link subcard"], tags: ["card", "parent", "hero", "mutation"], guidance: ["Prefer this dedicated Hero/sub-card relationship tool over raw dispatch and confirm both card and parent."], activeSafety: EXPLICIT_MUTATION_SAFETY },
  { name: "codecks_card_add_attachment", aliases: ["attach file", "add attachment", "upload attachment"], tags: ["card", "attachment", "file", "mutation"], guidance: ["Confirm the exact card and local file before attaching it; attachment contents may be shared with tracker users."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Confirm the exact card and file path.` },
  { name: "codecks_card_update", aliases: ["update card", "edit card", "change card content", "assign milestone"], tags: ["card", "update", "content", "metadata", "mutation"], guidance: ["Update only explicitly requested card fields and inspect the target first when it is ambiguous.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} ${CARD_REFERENCE_SAFETY}` },
  ...
  { name: "codecks_card_bulk_create", aliases: ["bulk create cards", "import cards", "csv card import"], tags: ["card", "bulk", "create", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first, review duplicate candidates, and apply only after explicit approval."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Default to dryRun=true and apply only after approval of the preview and duplicate review.` },
  { name: "codecks_card_bulk_update", aliases: ["bulk update cards", "import card updates", "csv card update"], tags: ["card", "bulk", "update", "import", "mutation", "preview"], guidance: ["Run with dryRun=true first and apply broad tracker edits only after explicit approval of the preview."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Default to dryRun=true and apply only after approval of the preview.` },
  { name: "codecks_card_update_effort", aliases: ["set effort", "update effort", "estimate card", "apply effort"], tags: ["card", "effort", "update", "mutation"], guidance: ["Use only after eligible cards and exclusions have been previewed and the user explicitly approved target effort values."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Requires a reviewed candidate preview and approval of the exact effort value.` },

  { name: "codecks_card_update_status", aliases: ["update status", "change status", "mark done", "start card"], tags: ["card", "status", "lifecycle", "mutation"], guidance: ["Documentation-card status writes and direct Hero-card starts are unsupported.", "Local implementation completion is not permission to mark a card done; require explicit status-change authorization."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Local implementation completion is not permission to mark done. Documentation cards cannot transition and Hero cards cannot be started directly.` },
  { name: "codecks_card_update_priority", aliases: ["update priority", "set priority", "change priority"], tags: ["card", "priority", "metadata", "mutation"], guidance: ["Confirm the exact card and requested priority before changing it."], activeSafety: EXPLICIT_MUTATION_SAFETY },

  { name: "codecks_deck_update", aliases: ["update deck", "edit deck description", "clear deck description"], tags: ["deck", "description", "update", "mutation"], guidance: ["This tool edits only deck descriptions. Require explicit edit intent; numeric deckId values are account sequences, and clearDescription=true or an empty string clears the description."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Only deck descriptions are supported; resolve the exact target before editing.` },
  ...
  { name: "codecks_run_update", aliases: ["update run", "edit run label", "edit run description", "clear run label"], tags: ["run", "sprint", "update", "mutation"], guidance: ["Require explicit Run and field intent. Custom labels map to Sprint name; use clearCustomLabel=true to clear one."], activeSafety: EXPLICIT_MUTATION_SAFETY },
  { name: "codecks_card_update_run", aliases: ["assign card to run", "remove card from run", "change card run"], tags: ["run", "card", "assign", "update", "mutation"], guidance: ["Require explicit card and Run assignment/removal intent; use clearRun=true to remove a card from its Run."], activeSafety: EXPLICIT_MUTATION_SAFETY },
  ...
  { name: "codecks_card_add_comment", aliases: ["add comment", "new comment", "open comment"], tags: ["conversation", "comment", "create", "mutation"], guidance: ["Open a new general comment thread only when the user explicitly asks; do not use it for routine follow-up or to reply.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Never use a new comment as a reply. ${CARD_REFERENCE_SAFETY}` },
  { name: "codecks_card_add_review", aliases: ["add review", "open review", "new review"], tags: ["conversation", "review", "create", "mutation"], guidance: ["Codecks allows one open review per card. For follow-up, discover and reply to the existing review instead of opening another thread.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Do not open a second review or use a new review for follow-up; inspect existing threads first. ${CARD_REFERENCE_SAFETY}` },
  { name: "codecks_card_add_blocker", aliases: ["add blocker", "open blocker", "block card"], tags: ["conversation", "blocker", "create", "mutation"], guidance: ["Open a blocker conversation only with explicit intent; Review and Blocker are mutually exclusive while open.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Review and Blocker are mutually exclusive while open.` },
  { name: "codecks_card_reply_resolvable", aliases: ["reply to thread", "reply to review", "reply to comment", "follow up review"], tags: ["conversation", "resolvable", "reply", "mutation"], guidance: ["Reply to an existing thread, preferring resolvableId. If only a card is known and multiple threads may match, list resolvables first.", "Closed threads must be reopened before replying.", CARD_REFERENCE_SAFETY], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Reply only to an identified existing thread; discover ambiguous threads and reopen closed threads first. ${CARD_REFERENCE_SAFETY}` },
  { name: "codecks_card_edit_resolvable_entry", aliases: ["edit comment entry", "edit review entry", "edit thread entry"], tags: ["conversation", "resolvable", "edit", "mutation"], guidance: ["Only entries authored by the logged-in user can be edited; confirm the exact entry and replacement text."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Only edit an entry authored by the logged-in user.` },
  { name: "codecks_card_close_resolvable", aliases: ["close thread", "resolve review", "close blocker", "close comment"], tags: ["conversation", "resolvable", "close", "mutation"], guidance: ["Confirm the exact open thread before closing it."], activeSafety: EXPLICIT_MUTATION_SAFETY },
  { name: "codecks_card_reopen_resolvable", aliases: ["reopen thread", "reopen review", "reopen comment", "reopen blocker"], tags: ["conversation", "resolvable", "reopen", "mutation"], guidance: ["Reopen a known closed thread before replying to it."], activeSafety: EXPLICIT_MUTATION_SAFETY },
  { name: "codecks_card_add_block", aliases: ["deprecated add block"], tags: ["conversation", "blocker", "deprecated"], guidance: ["Deprecated alias. Prefer codecks_card_add_blocker."], activeSafety: `${EXPLICIT_MUTATION_SAFETY} Deprecated alias; prefer codecks_card_add_blocker.`, exactOnly: true },
