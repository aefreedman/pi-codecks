...
Bulk updates support `clearMilestone`, `clearAssignee`, `clearEffort`, `clearRun`, and `clearParent`: set the flag to `true` to remove that value. Deck removal is unavailable: `clearDeck: true` rejects the entire batch before any requests, with an explanation. Live validation of `cards/update` with `deckId: null` returned HTTP 500; use the Codecks UI until the removal contract is verified. Assigning a deck remains supported. Clearing the assignee of an already deckless card makes it a note visible only to its creator; explain that visibility consequence in the preview before approval. Do not combine a clear flag with its corresponding assignment field. Omitted fields stay unchanged; false clear flags alone are rejected as no-ops. Use `tags: []`, `priority: "none"`, or `effort: null` to clear those values (`clearEffort` is an explicit alternative). For example, preview milestone removal with `updates: [{ cardId: "$123", clearMilestone: true }]` and `dryRun: true`, then apply with the returned fingerprint.
