...
Bulk create deduplicates normalized titles and uses up to four paged title-contains probes, with client-side exact-title and resolved Deck/milestone scope matching. Larger, rejected, or incomplete narrow probes use one bounded accessible-account fallback; Deck filtering is only an optional optimization and milestone filtering is not required. `duplicatePolicy` defaults to `required` for dry-run and `best_effort` for apply. `required` blocks incomplete discovery, `best_effort` prominently reports scan-limit incompleteness but can apply, and explicit `skip` performs no discovery. Infrastructure, cancellation, timeout, or queue failures always remain blocking. `duplicateLimit: 0` suppresses returned candidates, not discovery. A `required` parent-local guarantee is not available yet.

Default `verification: "none"` performs zero post-create reads and reports dispatch-returned `cardRef` / card ID immediately. Opt into `verification: "identity"` only when an exact identity read-back is useful; it makes at most one read for each identifiable create, reports checked identity fields, and never changes dispatch certainty or retries a create. Use `outputMode: "detailed"` for compatible schema-v1 normalized diagnostics; compact schema-v2 apply results omit duplicated payloads, fingerprints, and raw dispatch values. Bulk update supports content/metadata plus effort, priority, tags, Run assignment/removal (`runId` / `clearRun`), and parent assignment/removal (`parentCardId` / `clearParent`) with indexed current/proposed and partial-apply results.
